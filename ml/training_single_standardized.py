...
def train(df, column, specification):
    ...
    #standardScaler_y = StandardScaler().fit(y.values.reshape(-1, 1))
    #y = standardScaler_y.transform(y.values.reshape(-1, 1))
    standardScaler_x = StandardScaler().fit(X.values)
    X = standardScaler_x.transform(X.values)

    print(X)
    print(y)

    #joblib.dump(standardScaler_y, 'std_sc_y.bin', compress=True)
    joblib.dump(standardScaler_x, 'std_sc_x.bin', compress=True)
...
def saveModels(models, folder, subfolder):
    for model, column in models:
        ...
        model.save('rig_standardized.h5')
    return True
